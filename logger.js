...
class Logger {
    ...
    error(message, extraDetail) {
        this._logLines.push(this._putRecord({level: 'ERROR', message, extraDetail}));
    }
    ...
    _putRecord({level, message, extraDetail = {}}) {
        ...
        const fullLogMessage = Object.assign({}, coreLogMessage, {level, message}, extraDetail);

        // eslint-disable-next-line no-console
        console.log(`${fullLogMessage.timestamp} ${fullLogMessage.level} ${fullLogMessage.message} ${fullLogMessage.extraDetail}`);
